...
export default function Orden({orden}) {
  ...
  const [preparadas, setPreparadas] = useState(pedido.map(() => ({ preparada: false, entregada: false })));
  ...
  const marcarPreparada = (index) => {
    setPreparadas((prevPreparadas) => {
      const nuevasPreparadas = [...prevPreparadas];
      nuevasPreparadas[index] = { preparada: true, entregada: false };
      return nuevasPreparadas;
    });
  };

  const marcarEntregada = (index) => {
    setPreparadas((prevPreparadas) => {
      const nuevasPreparadas = [...prevPreparadas];
      nuevasPreparadas[index] = { ...nuevasPreparadas[index], entregada: true };
      return nuevasPreparadas;
    });
  };
        ...
        {pedido.map((platillo, index) => (
          <div key={platillo.id} className="py-3 flex border-b last-of-type:border-0 items-center">
            ...
            <div className="p-5 space-y-2">
              ...
              {preparadas[index].preparada && !preparadas[index].entregada && (
                <p className="text-green-500 font-bold text-xl flex items-center">
                  <FontAwesomeIcon icon={faCheck} className="mr-2" />
                  Listo para entregar
                  <button className="bg-blue-600 hover:bg-blue-800 text-white mt-2 py-1 px-3 uppercase font-bold rounded-lg ml-2" type="button" onClick={() => marcarEntregada(index)}>
                    Marcar como entregado
                  </button>
                </p>
              )}

              {preparadas[index].entregada && (
